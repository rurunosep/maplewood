...
impl ScreenPos {
    fn from_point(point: Point) -> Self {
        Self { x: (point.x * TILE_SIZE as f64) as i32, y: (point.y * TILE_SIZE as f64) as i32 }
    }
}
...
pub fn render(
    ...
    for row in -100..100 {
        for col in -100..100 {
            ...
            {
                let cell_screen_pos =
                    ScreenPos::from_point(Point::new(col as f64, row as f64) - camera_top_left);
                let screen_rect =
    ...
    //     Point::new(entity::standing_cell(player).x as f64, entity::standing_cell(player).y as
    // f64)
    //         - camera_top_left,
    // );
    ...
    //     Point::new(entity::facing_cell(player).x as f64, entity::facing_cell(player).y as f64)
    //         - camera_top_left,
    // );
    ...
    let screen_rect = Rect::new(player_screen_top_left.x, player_screen_top_left.y, 16, 16);
    canvas.copy(spritesheet, sprite_rect, screen_rect).unwrap();

    // Draw message window
    if show_message_window {
        canvas.set_draw_color(Color::RGB(0, 0, 0));
        canvas.fill_rect(Rect::new(10, 16 * 12 - 60, 16 * 16 - 20, 50)).unwrap();
        let surface = font.render(message).blended_wrapped(Color::RGB(255, 255, 255), 0).unwrap();
        let texture_creator = canvas.texture_creator();
        let texture = texture_creator.create_texture_from_surface(&surface).unwrap();
        let TextureQuery { width, height, .. } = texture.query();
        canvas.copy(&texture, None, Rect::new(20, 16 * 12 - 55, width, height)).unwrap();
    }
